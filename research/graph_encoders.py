...
def encode_JSON_quiver_rep_arrays(Q_as_dict):
    ...
    if len(Q_to_file['graph']) > 0:
        ...
        for key in graph_keys:
            ...
            if (isinstance(Q_to_file['graph'][key], list) and len(Q_to_file['graph'][key]) > 0):
                b = Q_to_file['graph'][key][0]
                if isinstance(b, np.ndarray):
                    array_list_as_list_of_lists = [b.tolist() for b in Q_to_file['graph'][key]]
    ...
    for key in node_keys:
        ...
        if (isinstance(Q_to_file['nodes'][0][key], list) and len(Q_to_file['nodes'][0][key]) > 0):
    ...
    for key in edge_keys:
        ...
        if (isinstance(Q_to_file['edges'][0][key], list) and len(Q_to_file['edges'][0][key]) > 0):
    ...
    Q_to_file['graph']['list_of_nparrays_graph_attributes'] = list_of_nparrays_graph_attributes
    Q_to_file['graph']['nparray_node_attributes'] = nparray_node_attributes
    Q_to_file['graph']['list_of_nparrays_node_attributes'] = list_of_nparrays_node_attributes
    Q_to_file['graph']['nparray_edge_attributes'] = nparray_edge_attributes
    Q_to_file['graph']['list_of_nparrays_edge_attributes'] = list_of_nparrays_edge_attributes
...
def decode_JSON_quiver_rep_arrays(Q_from_file):
    ...
    list_of_nparrays_graph_attributes = Q_as_dict['graph']['list_of_nparrays_graph_attributes']
    nparray_node_attributes = Q_as_dict['graph']['nparray_node_attributes']
    list_of_nparrays_node_attributes = Q_as_dict['graph']['list_of_nparrays_node_attributes']
    nparray_edge_attributes = Q_as_dict['graph']['nparray_edge_attributes']
    list_of_nparrays_edge_attributes = Q_as_dict['graph']['list_of_nparrays_edge_attributes']
    ...
    for key in list_of_nparrays_node_attributes:
        for i in range(len(Q_as_dict['nodes'])):
            Q_as_dict['nodes'][i][key] = [np.array(b) for b in Q_as_dict['nodes'][i][key]]
    ...
    for key in list_of_nparrays_edge_attributes:
        for i in range(len(Q_as_dict['edges'])):
            Q_as_dict['edges'][i][key] = [np.array(b) for b in Q_as_dict['edges'][i][key]]
